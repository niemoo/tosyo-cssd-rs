<x-layouts.app title="Kategori Instrumen">

    <x-slot name="pageHeader">Kategori Instrumen</x-slot>
    <x-slot name="pageSubHeader">Kelola kategori instrumen CSSD</x-slot>
    <x-slot name="pageActions">
        @can('instrument-categories.create')
            <a href="{{ route('instrument-categories.create') }}"
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($categories as $category)
                        <tr class="transition hover:bg-gray-50/50 {{ $category->trashed() ? 'opacity-50' : '' }}">
                            ...
                            <td class="px-5 py-3.5">
                                <span
                                    class="line-clamp-1 text-xs text-gray-500">{{ $category->description ?? '—' }}</span>
                            </td>
                            ...
                            <td class="px-5 py-3.5">
                                <x-kebab-menu>
                                    @if ($category->trashed())
                                        @can('instrument-categories.delete')
                                            <form method="POST"
                                                action="{{ route('instrument-categories.restore', $category->id) }}">
                                                @csrf @method('PATCH')
                                                <button type="submit"
                                    ...
                                    @else
                                        @can('instrument-categories.view')
                                            <a href="{{ route('instrument-categories.show', $category) }}"
                                        ...
                                        @can('instrument-categories.edit')
                                            <a href="{{ route('instrument-categories.edit', $category) }}"
                                                class="flex items-center gap-2.5 px-4 py-2.5 text-xs font-medium text-gray-700 hover:bg-gray-50">
                                                <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor"
                                                    stroke-width="2" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ $multiHospital ? 6 : 5 }}">
                                <x-empty-state title="Belum ada kategori instrumen"
                                    description="Tambahkan kategori untuk mengelompokkan instrumen CSSD."
                                    action-label="Tambah Kategori" action-route="instrument-categories.create"
                                    icon="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
                            </td>
                        </tr>
                    @endforelse
    ...
    @foreach ($categories as $category)
        @unless ($category->trashed())
            @can('instrument-categories.delete')
                <form method="POST" action="{{ route('instrument-categories.destroy', $category) }}"
                    id="form-delete-{{ $category->id }}">
                    @csrf @method('DELETE')
                </form>
                <x-modal-confirm :id="'confirm-' . $category->id" type="danger" title="Hapus Kategori?" :description="'Kategori ' .
                    $category->name .
                    ' akan dihapus. Pastikan tidak ada instrumen yang menggunakan kategori ini.'"
                    confirm-text="Ya, Hapus" :form-id="'form-delete-' . $category->id" />
            @endcan

            @can('instrument-categories.edit')
                <form method="POST" action="{{ route('instrument-categories.toggle-active', $category) }}"
