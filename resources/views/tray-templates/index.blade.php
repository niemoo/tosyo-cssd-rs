<x-layouts.app title="Tray Template">

    <x-slot name="pageHeader">Tray Template</x-slot>
    <x-slot name="pageSubHeader">Kelola template susunan instrumen tray</x-slot>
    <x-slot name="pageActions">
        @can('tray-templates.create')
            <a href="{{ route('tray-templates.create') }}"
    ...
    <div class="mb-6 grid grid-cols-3 gap-4">
        <x-stat-card label="Total Template" :value="$stats['total']" color="teal"
    ...
    <div class="overflow-hidden rounded-xl border border-gray-200 bg-white">
        <div class="flex items-center justify-between border-b border-gray-50 px-5 py-3.5">
            <p class="text-xs text-gray-400">
                Menampilkan <span
                    class="font-semibold text-gray-700">{{ $templates->firstItem() }}–{{ $templates->lastItem() }}</span>
                dari <span class="font-semibold text-gray-700">{{ $templates->total() }}</span> template
            </p>
        </div>
            ...
            <table class="w-full min-w-[640px]">
                <thead>
                    <tr class="border-b border-gray-50 bg-gray-50/50">
                        ...
                        <th class="px-5 py-3 text-left text-[11px] font-semibold uppercase tracking-wider">
                            <x-sort-header column="name" label="Nama Template" :current-sort="$sortBy" :current-direction="$sortDir" />
                        </th>
                        @if ($multiHospital)
                            <th
                        ...
                        <th class="px-5 py-3 text-center text-[11px] font-semibold uppercase tracking-wider">
                            <x-sort-header column="is_lockable" label="Lockable" :current-sort="$sortBy" :current-direction="$sortDir" />
                        </th>
                        <th class="px-5 py-3 text-center text-[11px] font-semibold uppercase tracking-wider">
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($templates as $template)
                        <tr class="transition hover:bg-gray-50/50 {{ $template->trashed() ? 'opacity-50' : '' }}">
                            ...
                            <td class="px-5 py-3.5">
                                ...
                                @if ($template->description)
                                    <div class="line-clamp-1 text-xs text-gray-400">{{ $template->description }}</div>
                                @endif
                                @if ($template->trashed())
                                    <span
                                        class="mt-1 inline-block rounded-full bg-red-100 px-2 py-0.5 text-[10px] font-semibold text-red-600">
                                        Dihapus {{ $template->deleted_at->diffForHumans() }}
                                    </span>
                                @endif
                            </td>
                            @if ($multiHospital)
                                <td class="px-5 py-3.5">
                                    <div class="text-xs font-medium text-gray-700">{{ $template->hospital->name }}
                                    </div>
                                    <div class="text-[10px] text-gray-400">{{ $template->hospital->code }}</div>
                                </td>
                            @endif
                            <td class="px-5 py-3.5 text-center">
                                <span
                                    class="text-sm font-semibold text-gray-900">{{ $template->instruments_count }}</span>
                                <span class="text-xs text-gray-400"> jenis</span>
                            </td>
                            <td class="px-5 py-3.5 text-center">
                                @if ($template->is_lockable)
                            ...
                            <td class="px-5 py-3.5 text-center">
                                ...
                                @else
                                    <x-badge :color="$template->is_active ? 'green' : 'red'" dot>
                            ...
                            <td class="px-5 py-3.5">
                                <x-kebab-menu>
                                    @if ($template->trashed())
                                        @can('tray-templates.delete')
                                            <form method="POST"
                                                action="{{ route('tray-templates.restore', $template->id) }}">
                                                @csrf @method('PATCH')
                                                <button type="submit"
                                    ...
                                    @else
                                        @can('tray-templates.view')
                                            <a href="{{ route('tray-templates.show', $template) }}"
                                        ...
                                        @can('tray-templates.edit')
                                            <a href="{{ route('tray-templates.edit', $template) }}"
                                                class="flex items-center gap-2.5 px-4 py-2.5 text-xs font-medium text-gray-700 hover:bg-gray-50">
                                                <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor"
                                                    stroke-width="2" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                            ...
                                            <button type="button"
                                                onclick="document.getElementById('confirm-toggle-{{ $template->id }}').showModal()"
                                                class="flex w-full items-center gap-2.5 px-4 py-2.5 text-xs font-medium hover:bg-gray-50
                                                           {{ $template->is_active ? 'text-orange-600' : 'text-green-600' }}">
                                                <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor"
                                                    stroke-width="2" viewBox="0 0 24 24">
                                        ...
                                        @can('tray-templates.delete')
                                            ...
                                            <button type="button"
                                                onclick="document.getElementById('confirm-{{ $template->id }}').showModal()"
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ $multiHospital ? 7 : 6 }}">
                                <x-empty-state title="Belum ada tray template"
                                    description="Tambahkan template untuk mendefinisikan susunan instrumen tray."
                                    action-label="Tambah Template" action-route="tray-templates.create"
                                    icon="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                            </td>
                        </tr>
                    @endforelse
    ...
    @foreach ($templates as $template)
        @unless ($template->trashed())
            @can('tray-templates.delete')
                <form method="POST" action="{{ route('tray-templates.destroy', $template) }}"
                    id="form-delete-{{ $template->id }}">
                    @csrf @method('DELETE')
                </form>
                <x-modal-confirm :id="'confirm-' . $template->id" type="danger" title="Hapus Template?" :description="'Template ' .
                    $template->name .
                    ' akan dihapus. Pastikan tidak ada tray aktif yang menggunakan template ini.'"
                    confirm-text="Ya, Hapus" :form-id="'form-delete-' . $template->id" />
            @endcan

            @can('tray-templates.edit')
                <form method="POST" action="{{ route('tray-templates.toggle-active', $template) }}"
                    id="form-toggle-{{ $template->id }}">
                    @csrf @method('PATCH')
                </form>
                <x-modal-confirm :id="'confirm-toggle-' . $template->id" type="warning" :title="$template->is_active ? 'Nonaktifkan Template?' : 'Aktifkan Template?'" :description="$template->is_active
                    ? $template->name . ' akan dinonaktifkan.'
                    : $template->name . ' akan diaktifkan kembali.'" :confirm-text="$template->is_active ? 'Ya, Nonaktifkan' : 'Ya, Aktifkan'"
                    :form-id="'form-toggle-' . $template->id" />
            @endcan
        @endunless
    @endforeach
