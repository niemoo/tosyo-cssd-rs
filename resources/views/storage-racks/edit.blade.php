<x-layouts.app title="Edit — {{ $storageRack->name }}">

    <x-slot name="backButton">
        <a href="{{ route('storage-racks.show', $storageRack) }}"
    ...
    <x-slot name="breadcrumb">
        ...
        <a href="{{ route('storage-racks.show', $storageRack) }}"
            class="hover:text-gray-600">{{ $storageRack->name }}</a>
        <svg class="h-3 w-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" d="M9 18l6-6-6-6" />
        </svg>
    ...
    <x-slot name="pageHeader">Edit Rak</x-slot>
    <x-slot name="pageSubHeader">{{ $storageRack->name }}</x-slot>

    <form method="POST" action="{{ route('storage-racks.update', $storageRack) }}">
        @csrf @method('PUT')
                    ...
                    <div class="grid grid-cols-1 gap-4 p-5 sm:grid-cols-2">
                        ...
                        <div class="sm:col-span-2">
                            <label
                                ...
                                Sakit</label>
                            <input type="text" value="{{ $storageRack->hospital->name }}" disabled
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" name="name" value="{{ old('name', $storageRack->name) }}"
                        ...
                        <div>
                            ...
                            <input type="text" value="{{ $storageRack->code }}" disabled
                                class="w-full cursor-not-allowed rounded-lg border border-gray-200 bg-gray-100 px-3.5 py-2.5 text-sm text-gray-400" />
                        ...
                        <div>
                            <label
                                ...
                                (Tray)</label>
                            <input type="number" name="capacity" value="{{ old('capacity', $storageRack->capacity) }}"
                        ...
                        <div class="sm:col-span-2">
                            <label
                                class="mb-1.5 block text-[11px] font-semibold uppercase tracking-wider text-gray-500">Deskripsi
                                Lokasi</label>
                            <input type="text" name="location_desc"
                                value="{{ old('location_desc', $storageRack->location_desc) }}"
                                placeholder="cth. Ruang Steril Lantai 2"
                                class="w-full rounded-lg border border-gray-200 bg-gray-50 px-3.5 py-2.5 text-sm text-gray-900 placeholder-gray-300 transition
                                          focus:border-primary-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary-400/20" />
                        </div>
                ...
                <div class="overflow-hidden rounded-xl border border-gray-200 bg-white">
                    ...
                    <div class="p-5">
                        <div
                            ...
                            <div>
                                <div class="text-sm font-medium text-gray-900">Rak Aktif</div>
                                <div class="text-xs text-gray-400">Dapat digunakan dalam sistem</div>
                            </div>
                            <label class="relative inline-flex cursor-pointer items-center">
                                <input type="checkbox" name="is_active" value="1"
                                    {{ old('is_active', $storageRack->is_active) ? 'checked' : '' }}
                                    class="peer sr-only" />
                                <div
