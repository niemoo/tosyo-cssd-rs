<x-layouts.app title="Consumable">

    <x-slot name="pageHeader">Consumable</x-slot>
    <x-slot name="pageSubHeader">Kelola master data consumable CSSD</x-slot>
    ...
    <div class="mb-6 grid grid-cols-3 gap-4">
        <x-stat-card label="Total Consumable" :value="$stats['total']" color="teal"
    ...
    <div class="mb-4 flex flex-wrap items-center gap-3 rounded-xl border border-gray-100 bg-white p-4">
        <form method="GET" class="flex flex-1 flex-wrap items-center gap-3">
            ...
            @if (request()->hasAny(['search', 'status', 'show_deleted', 'hospital_id', 'category_id', 'stock']))
                <a href="{{ route('consumables.index') }}" class="text-sm text-gray-400 hover:text-gray-600">Reset</a>
            @endif
        </form>
    </div>
    ...
    <div class="overflow-hidden rounded-xl border border-gray-100 bg-white">
        <div class="flex items-center justify-between border-b border-gray-50 px-5 py-3.5">
            <p class="text-xs text-gray-400">
                Menampilkan <span
                    class="font-semibold text-gray-700">{{ $consumables->firstItem() }}–{{ $consumables->lastItem() }}</span>
                dari <span class="font-semibold text-gray-700">{{ $consumables->total() }}</span> consumable
            </p>
        </div>
            ...
            <table class="w-full min-w-[800px]">
                <thead>
                    <tr class="border-b border-gray-50 bg-gray-50/50">
                        ...
                        <th class="px-5 py-3 text-center text-[11px] font-semibold uppercase tracking-wider">
                            <x-sort-header column="unit" label="Satuan" :current-sort="$sortBy" :current-direction="$sortDir" />
                        </th>
                        <th class="px-5 py-3 text-center text-[11px] font-semibold uppercase tracking-wider">
                            Stok
                        </th>
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($consumables as $consumable)
                        <tr class="transition hover:bg-gray-50/50 {{ $consumable->trashed() ? 'opacity-50' : '' }}">
                            ...
                            <td class="px-5 py-3.5">
                                <div class="font-semibold text-gray-900">{{ $consumable->name }}</div>
                                @if ($consumable->trashed())
                                    <span
                                        class="mt-1 inline-block rounded-full bg-red-100 px-2 py-0.5 text-[10px] font-semibold text-red-600">
                                        Dihapus {{ $consumable->deleted_at->diffForHumans() }}
                                    </span>
                                @endif
                            </td>
                            @if ($multiHospital)
                                <td class="px-5 py-3.5">
                                    <div class="text-xs font-medium text-gray-700">{{ $consumable->hospital->name }}
                                    </div>
                                    <div class="text-[10px] text-gray-400">{{ $consumable->hospital->code }}</div>
                                </td>
                            @endif
                            <td class="px-5 py-3.5">
                                <x-badge color="blue">{{ $consumable->category->name }}</x-badge>
                            </td>
                            <td class="px-5 py-3.5 text-center">
                                <span
                                    class="text-xs font-medium text-gray-600">{{ $units[$consumable->unit] ?? $consumable->unit }}</span>
                            </td>
                            <td class="px-5 py-3.5 text-center">
                                @php
                                ...
                                @endphp
                                <div class="text-sm font-semibold {{ $isLow ? 'text-red-600' : 'text-gray-900' }}">
                                    {{ number_format($currentStock) }}
                                </div>
                                <div class="text-[10px] text-gray-400">min. {{ $consumable->minimum_stock }}</div>
                                @if ($isLow && !$consumable->trashed())
                                    <x-badge color="amber">Menipis</x-badge>
                                @endif
                            </td>
                            <td class="px-5 py-3.5 text-center">
                                @if ($consumable->trashed())
                                ...
                                <x-kebab-menu>
                                    @if ($consumable->trashed())
                                        @can('consumables.delete')
                                            <form method="POST"
                                                action="{{ route('consumables.restore', $consumable->id) }}">
                                                @csrf @method('PATCH')
                                                <button type="submit"
                                    ...
                                    @else
                                        ...
                                        @can('consumables.edit')
                                            ...
                                            <button type="button"
                                                onclick="document.getElementById('confirm-toggle-{{ $consumable->id }}').showModal()"
                                                class="flex w-full items-center gap-2.5 px-4 py-2.5 text-xs font-medium hover:bg-gray-50
                                                           {{ $consumable->is_active ? 'text-orange-600' : 'text-green-600' }}">
                                                <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor"
                                                    stroke-width="2" viewBox="0 0 24 24">
                                                ...
                                                {{ $consumable->is_active ? 'Nonaktifkan' : 'Aktifkan' }}
                                            </button>
                                        @endcan
                                        ...
                                        @can('consumables.delete')
                                            ...
                                            <button type="button"
                                                onclick="document.getElementById('confirm-{{ $consumable->id }}').showModal()"
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ $multiHospital ? 8 : 7 }}">
                                <x-empty-state title="Belum ada consumable"
                                    description="Tambahkan consumable untuk mulai mengelola stok CSSD."
                                    action-label="Tambah Consumable" action-route="consumables.create"
                                    icon="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
                            </td>
                        </tr>
                    @endforelse
        ...
        @if ($consumables->hasPages())
            <div class="border-t border-gray-50 px-5 py-3.5">
                {{ $consumables->links('components.pagination') }}
            </div>
        @endif
    </div>
    ...
    @foreach ($consumables as $consumable)
        @unless ($consumable->trashed())
            @can('consumables.delete')
                <form method="POST" action="{{ route('consumables.destroy', $consumable) }}"
                    id="form-delete-{{ $consumable->id }}">
                    @csrf @method('DELETE')
                </form>
                <x-modal-confirm :id="'confirm-' . $consumable->id" type="danger" title="Hapus Consumable?" :description="'Consumable ' . $consumable->name . ' akan dihapus.'"
                    confirm-text="Ya, Hapus" :form-id="'form-delete-' . $consumable->id" />
            @endcan

            @can('consumables.edit')
                <form method="POST" action="{{ route('consumables.toggle-active', $consumable) }}"
                    id="form-toggle-{{ $consumable->id }}">
                    @csrf @method('PATCH')
                </form>
                <x-modal-confirm :id="'confirm-toggle-' . $consumable->id" type="warning" :title="$consumable->is_active ? 'Nonaktifkan Consumable?' : 'Aktifkan Consumable?'" :description="$consumable->is_active
                    ? $consumable->name . ' akan dinonaktifkan.'
                    : $consumable->name . ' akan diaktifkan kembali.'" :confirm-text="$consumable->is_active ? 'Ya, Nonaktifkan' : 'Ya, Aktifkan'"
                    :form-id="'form-toggle-' . $consumable->id" />
            @endcan
        @endunless
    @endforeach
