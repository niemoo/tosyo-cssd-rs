<x-layouts.app title="Edit — {{ $consumable->name }}">
    ...
    <x-slot name="breadcrumb">
        ...
        <a href="{{ route('consumables.show', $consumable) }}" class="hover:text-gray-600">{{ $consumable->name }}</a>
        <svg class="h-3 w-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" d="M9 18l6-6-6-6" />
        </svg>
    ...
    <x-slot name="pageHeader">Edit Consumable</x-slot>
    <x-slot name="pageSubHeader">{{ $consumable->name }}</x-slot>
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
                    ...
                    <div class="grid grid-cols-1 gap-4 p-5 sm:grid-cols-2">
                        ...
                        <div class="sm:col-span-2">
                            <label
                                ...
                                Sakit</label>
                            <input type="text" value="{{ $consumable->hospital->name }}" disabled
                        ...
                        <div>
                            ...
                            <input type="text" value="{{ $consumable->code }}" disabled
                                class="w-full cursor-not-allowed rounded-lg border border-gray-100 bg-gray-100 px-3.5 py-2.5 text-sm text-gray-400" />
                        ...
                        <div>
                            ...
                            <select name="unit"
                                ...
                                @foreach ($units as $key => $label)
                                    <option value="{{ $key }}"
                                        {{ old('unit', $consumable->unit) === $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input type="number" name="minimum_stock"
                                value="{{ old('minimum_stock', $consumable->minimum_stock) }}" min="0"
            ...
            <div class="space-y-4">
                ...
                <div class="rounded-xl border border-gray-100 bg-white p-5">
                    <h4 class="mb-4 text-[11px] font-bold uppercase tracking-wider text-gray-400">Stok Saat Ini</h4>
                    <div class="text-center">
                        @php $isLow = $consumable->isLowStock(); @endphp
                        <div class="text-3xl font-bold {{ $isLow ? 'text-red-600' : 'text-gray-900' }}">
                            {{ number_format($consumable->current_stock) }}
                        </div>
                        <div class="mt-1 text-xs text-gray-400">{{ $units[$consumable->unit] ?? $consumable->unit }}
                        </div>
                        @if ($isLow)
                            <div class="mt-2">
                                <x-badge color="amber">Stok Menipis</x-badge>
                            </div>
                        @endif
                    </div>
                </div>
