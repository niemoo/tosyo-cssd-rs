<x-layouts.app title="{{ $sterilizer->name }}">
    ...
    <x-slot name="breadcrumb">
        ...
        <span class="text-gray-700">{{ $sterilizer->name }}</span>
    </x-slot>

    <x-slot name="pageHeader">{{ $sterilizer->name }}</x-slot>
    <x-slot name="pageSubHeader">{{ $sterilizer->code }} · {{ $sterilizer->hospital->name }}</x-slot>
        ...
        <div class="space-y-4 lg:col-span-2">
            ...
            <div class="overflow-hidden rounded-xl border border-gray-200 bg-white">
                ...
                <div class="divide-y divide-gray-50">
                    @foreach ([['label' => 'Nama', 'value' => $sterilizer->name], ['label' => 'Kode', 'value' => $sterilizer->code], ['label' => 'Serial Number', 'value' => $sterilizer->serial_number ?? '-'], ['label' => 'Kapasitas', 'value' => $sterilizer->capacity ? $sterilizer->capacity . ' tray' : '-'], ['label' => 'Rumah Sakit', 'value' => $sterilizer->hospital->name]] as $item)
            ...
            <div class="overflow-hidden rounded-xl border border-gray-200 bg-white">
                ...
                <div class="divide-y divide-gray-50">
                    <div class="flex items-start justify-between px-5 py-3">
                        <span class="w-36 shrink-0 text-xs text-gray-400">Maintenance Terakhir</span>
                        <span class="text-right text-sm font-medium text-gray-700">
                            {{ $sterilizer->last_maintenance_at?->format('d M Y') ?? '-' }}
                        </span>
                    </div>
                    <div class="flex items-center justify-between px-5 py-3">
                        <span class="text-xs text-gray-400">Maintenance Berikutnya</span>
                        <div class="text-right">
                            @if ($sterilizer->next_maintenance_at)
                                <div
                                    class="text-sm font-medium {{ $sterilizer->isMaintenanceDue() ? 'text-red-600' : 'text-gray-700' }}">
                                    {{ $sterilizer->next_maintenance_at->format('d M Y') }}
                                </div>
                                @if ($sterilizer->isMaintenanceDue())
                                    <x-badge color="red">Segera lakukan maintenance</x-badge>
                                @else
                                    <div class="text-xs text-gray-400">
                                        {{ $sterilizer->next_maintenance_at->diffForHumans() }}</div>
                                @endif
                            @else
