<x-layouts.app title="Edit — {{ $consumableCategory->name }}">

    <x-slot name="backButton">
        <a href="{{ route('consumable-categories.show', $consumableCategory) }}"
    ...
    <x-slot name="breadcrumb">
        ...
        <a href="{{ route('consumable-categories.show', $consumableCategory) }}"
            class="hover:text-gray-600">{{ $consumableCategory->name }}</a>
        <svg class="h-3 w-3" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" d="M9 18l6-6-6-6" />
        </svg>
    ...
    <x-slot name="pageSubHeader">{{ $consumableCategory->name }}</x-slot>

    <form method="POST" action="{{ route('consumable-categories.update', $consumableCategory) }}">
        @csrf @method('PUT')
                    ...
                    <div class="grid grid-cols-1 gap-4 p-5 sm:grid-cols-2">
                        ...
                        <div class="sm:col-span-2">
                            <label
                                ...
                                Sakit</label>
                            <input type="text" value="{{ $consumableCategory->hospital->name }}" disabled
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" name="name" value="{{ old('name', $consumableCategory->name) }}"
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" value="{{ $consumableCategory->code }}" disabled
                                class="w-full cursor-not-allowed rounded-lg border border-gray-200 bg-gray-100 px-3.5 py-2.5 text-sm text-gray-400" />
                        ...
                        <div
                            ...
                            <label class="relative inline-flex cursor-pointer items-center">
                                <input type="checkbox" name="is_active" value="1"
                                    {{ old('is_active', $consumableCategory->is_active) ? 'checked' : '' }}
                                    class="peer sr-only" />
                                <div
